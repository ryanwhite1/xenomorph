...
grain_size = 1        # angstroms
dist = 1700             # dust distance, au
tens_str = 1e9          # dust tensile strength
...
crit_grain_size = 0.03197 * ((mean_lambda / 0.5)**-1.7 * (luminosity / (10**9 * (dist / pc_to_AU)**2))**(-1/3) * (tens_str / 10**9)**1/2)**(1/2.7)
print("Critical grain size =", crit_grain_size, "* 0.1micron =", crit_grain_size * 10**3, "A")
...
grain_grain_timescale = (np.pi * grain_size**2 * (density / (1e5 * hydrogen_mass)) * drift_velocity)**-1  / yr_2_s
print(grain_grain_timescale)